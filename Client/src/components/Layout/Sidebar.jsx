...
import { HelpCircle, SettingsIcon, SunIcon, Calendar, BarChart2, ClipboardList, Layers, FileText, AlarmCheckIcon, LogOut } from 'lucide-react';
    ...
    return (
        <>
                ...
                <nav className="flex-1 flex flex-col gap-2">
                    {navItems.map((item) => (
                        <Link
                            ...
                            className={`group relative flex items-center gap-2 px-2.5 py-2 rounded-lg font-medium transition-all duration-200 ${
            ...
            <header className="md:hidden fixed top-0 left-0 right-0 h-14 bg-white/90 backdrop-blur border-b border-slate-200 z-40 flex items-center justify-between px-4">
                ...
                >
                    ...
                    <span className="block w-5">
                        <span className={`block h-[2px] bg-slate-800 transition-transform ${mobileOpen ? 'rotate-45 translate-y-[5px]' : ''}`}></span>
                        <span className={`block h-[2px] bg-slate-800 my-[5px] transition-opacity ${mobileOpen ? 'opacity-0' : 'opacity-100'}`}></span>
                        <span className={`block h-[2px] bg-slate-800 transition-transform ${mobileOpen ? '-rotate-45 -translate-y-[5px]' : ''}`}></span>
                    </span>
                </button>
            </header>
            ...
            >
                ...
                <nav className="px-3 py-3 flex flex-col gap-1 overflow-y-auto h-[calc(100%-56px-140px)]">
                    {navItems.map((item) => (
                        <Link
                            key={item.to}
                            to={item.to}
                            className={`group flex items-center gap-3 px-4 py-2.5 rounded-lg text-base font-medium border-l-4 transition-colors ${
                                location.pathname === item.to
                ...
                {/* Actions */}
                <div className="px-3 py-4 border-t border-slate-200 flex flex-col gap-3">
                    ...
                    {!isLoggedIn ? (
                        <>
                            <Link to="/signup" className="w-full">
                                <button aria-label="Sign up" className="w-full bg-indigo-600 text-white py-2.5 rounded-lg font-semibold shadow hover:bg-indigo-700 transition">
                                    Sign Up
                                </button>
                            </Link>
                            <Link to="/login" className="w-full">
                                <button aria-label="Log in" className="w-full bg-white text-indigo-600 border border-indigo-200 py-2.5 rounded-lg font-semibold shadow hover:bg-indigo-50 transition">
                                    Login
                                </button>
                            </Link>
                        </>
                        ...
                        <button
                            onClick={handleLogout}
                            aria-label="Logout"
                            className="w-full bg-red-600 text-white py-2.5 rounded-lg font-semibold shadow hover:bg-red-700 transition flex items-center justify-center gap-2"
                        >
                            <LogOut size={18} />
                            <span>Logout</span>
                        </button>
